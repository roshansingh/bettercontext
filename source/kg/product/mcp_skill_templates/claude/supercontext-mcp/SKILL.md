...
Use the legacy explicit shape only when the harness already supplied it: `repo`, `changed_files`, `changed_ranges`, `base_snapshot`, `base_checkout`, and `head_checkout`. Read `entry_resolution` first; derivation failures and `no_changes_detected` are explicit statuses, not empty evidence. Derived mode writes reusable caches under `repo_path/.supercontext/` and adds `.supercontext/` to the repo-local `.git/info/exclude`. The compact parent packet remains capped at 15K chars, and attached internal `followup_packets` may bring the combined response up to 20K chars.
